...
export function DjsPublicForm({
  id = "hire-form",
  title = "Complete the form",
  subtitle = "Tell us the details to hire one of our DJ producers",
  image,
  imageAlt = "DJ Producer",
  ...
  const isValidEventVisibility = (
    value: string
  ): value is EventVisibility => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (!isValidEventVisibility(formData.eventVisibility)) {
      setStatus("error")
      setMessage("Please select if the event is public or private.")
      return
    }
    ...
    try {
      await submitPublicForm({
        ...
        interest: "Contratar DJ",
        description: formData.description.trim(),
        hiring_place: formData.hiringPlace.trim(),
        approximate_date: formData.approximateDate.trim(),
        social_networks: formData.socialNetworks.trim() || undefined,
        event_visibility: formData.eventVisibility,
        performance_hours: formData.performanceHours.trim(),
        approximate_guests: formData.approximateGuests.trim(),
      })
      ...
      console.error("DJs public form error:", error)
      setStatus("error")
          ...
          <form onSubmit={handleSubmit} className="mx-auto w-full max-w-xl space-y-4">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <input
                ...
                value={formData.performanceHours}
                onChange={handleChange}
                min="1"
                required
                className="w-full rounded-lg border border-black/20 px-5 py-3.5 text-sm text-black placeholder:text-black/40 transition-colors focus:border-[oklch(0.55_0.18_20)] focus:outline-none"
              ...
              <input
                type="number"
                name="approximateGuests"
                placeholder="Approx. guests"
                value={formData.approximateGuests}
                onChange={handleChange}
                min="1"
                required
                className="w-full rounded-lg border border-black/20 px-5 py-3.5 text-sm text-black placeholder:text-black/40 transition-colors focus:border-[oklch(0.55_0.18_20)] focus:outline-none"
