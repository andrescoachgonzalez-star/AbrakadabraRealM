...
export function PublicForm({
  ...
  image,
  imageAlt = "Abrakadabra Realm",
  submitLabel = "SEND REQUEST",
}: PublicFormProps) {
  const FORM_INTEREST = interest
  ...
  const selectedCountry = useMemo(() => {
    return (
      countries.find(
        (country) =>
          country.code === formData.phoneCountryCode && country.iso2 === "CO"
      ) ||
      countries.find((country) => country.code === formData.phoneCountryCode) ||
      FALLBACK_COUNTRIES[0]
    )
  }, [countries, formData.phoneCountryCode])
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch(
        "https://api.abrakadabrarealm.com/api/public/forms",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
