...
export function WhatsAppWidget() {
  ...
  // --- Numbers (normalized for links) ---
  const WHATSAPP_TEXT = "Hi%2C%20I%27m%20interested%20in%20your%20luxury%20collection."
  const SMS_BODY = "Hi%2C%20I%27m%20interested%20in%20your%20luxury%20collection."
  ...
  const contactOptions = [
    // Colombia (principal)
    {
      ...
      href: `https://wa.me/${CONTACT_INFO.colombia.whatsappNumber}?text=${WHATSAPP_TEXT}`,
      icon: (
    ...
    },
    {
      ...
      href: `${CONTACT_INFO.colombia.smsHref}?body=${SMS_BODY}`,
      icon: (
    ...
    // USA (secundario)
    {
      ...
      label: "WhatsApp (US)",
      subtitle: "Chat instantly",
      href: `https://wa.me/${CONTACT_INFO.usa.whatsappNumber}?text=${WHATSAPP_TEXT}`,
      icon: (
        <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
          <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
        </svg>
    ...
    },
    {
      ...
      label: "SMS (US)",
      subtitle: "Text message",
      href: `${CONTACT_INFO.usa.smsHref}?body=${SMS_BODY}`,
      icon: (
        <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path
            strokeLinecap="round"
    ...
    >
      ...
      >
        <div className="relative ml-3 bg-primary rounded-2xl rounded-bl-none px-5 py-3.5 shadow-xl max-w-[210px]">
          <p className="text-primary-foreground text-xs leading-relaxed font-medium">{"How can we help you?"}</p>
          <p className="text-primary-foreground/70 text-[10px] mt-1">Reach out anytime</p>
        ...
        <div className="bg-card border border-border rounded-2xl shadow-2xl w-72 overflow-hidden ml-3">
          ...
          <div className="bg-foreground px-5 py-4 relative overflow-hidden">
            ...
            <div className="flex items-center justify-between relative z-10">
              <div>
                <p className="text-background font-serif text-base font-bold tracking-wide">Get in Touch</p>
                <p className="text-background/60 text-[11px] mt-0.5">How can we help you today?</p>
              </div>
              <button
          ...
          <div className="p-3 flex flex-col gap-2">
            {contactOptions.map((option, index) => (
              <a
                key={option.id}
                href={option.href}
                target={option.targetBlank ? "_blank" : undefined}
                rel={option.targetBlank ? "noopener noreferrer" : undefined}
                className={cn(
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-foreground tracking-wide">{option.label}</p>
          ...
          <div className="px-4 py-3 border-t border-border bg-secondary/30">
            <p className="text-[10px] text-muted-foreground text-center tracking-wider uppercase">Available 24/7 for you</p>
          </div>
        </div>
      </div>
      ...
      <button
        ...
        className={cn(
          ...
          isExpanded ? "bg-foreground hover:bg-foreground/90 w-7 rounded-r-[28px]" : "bg-primary hover:bg-primary/90 w-7 hover:w-9 rounded-r-[28px]"
        )}
      >
        {isExpanded ? (
